...
def register(username, password):
    """Registrasi pengguna baru. Account number di-generate otomatis di server."""
    try:
...
def login(username, password):
    """Login dan kembalikan ringkasan rekening."""
    try:
        hashed_password = hash_password(password)
        q = """
...
def get_account_info(account_number):
    """Ambil info rekening (username, saldo, created_at) berdasarkan nomor rekening."""
    try:
        q = """
